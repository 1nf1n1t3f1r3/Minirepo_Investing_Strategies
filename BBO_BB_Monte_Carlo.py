...
import numpy as np
import random
from BBO_BB import process_input_file  # Import the first script function

# Global variables. Same as strategy script
initial_capital = 100000  # Starting capital

allocation_per_trade = 0.2  # 50% of available capital per trade
...
def process_input_with_monte_carlo(input_file):
    trades_df = process_input_file(input_file, ma_length=100, upper_multiplier=3, lower_multiplier=1)

    output_file = input_file.split(".")[0] + "_BBO_BB-res.csv"
    monte_carlo_output_file = input_file.split(".")[0] + "_BBO_BB-res-MC.csv"
...
# MAKE SURE THE INPUT FILE IS THE SAME IN THE MAIN SCRIPT
