...
# ATR calculation with RMA (like PineScript)
def atr(df, length):
    high_low = df['High'] - df['Low']
    high_close = np.abs(df['High'] - df['Close'].shift())
    low_close = np.abs(df['Low'] - df['Close'].shift())
    tr = high_low.combine(high_close, np.maximum).combine(low_close, np.maximum)
    rma = tr.ewm(alpha=1 / length, adjust=False).mean()  # RMA calculation
    return rma


def moving_average(series, window):
    return series.ewm(span=window, adjust=False).mean()  # Commented out the EMA
    # return series.rolling(window=window).mean()  # Commented out the SMA


#  Keltner Channel calculation, with either SMA or EMA
def keltner_channels(df, length=20, atr_length=10, upper_multiplier=2.0, lower_multiplier=2.0):
    basis = moving_average(df['Close'], length)
    atr_value = atr(df, atr_length)
    upper_band = basis + (upper_multiplier * atr_value)
    lower_band = basis - (lower_multiplier * atr_value)
...
def get_combined_data(input_file):
    ...
    for _, row in input_data.iterrows():
        ...
        try:
            start_date_extended = (datetime.strptime(start_date, '%Y-%m-%d') - timedelta(days=100)).strftime('%Y-%m-%d')
            ...
            # Verify Band_Position exists after keltner_channels calculation
...
def process_trades_day_by_day(combined_data, min_days_below_upper, min_band_position_threshold, max_band_position_threshold):
    ...
    for current_date, daily_data in combined_data.groupby('Date'):
        ...
        for _, row in daily_data.iterrows():
            ...
            if (
                    ...
                    and min_band_position_threshold <= band_position <= max_band_position_threshold
            # Band_Position threshold
            ):
...
def process_input_file(input_file, ma_length, upper_multiplier, lower_multiplier, min_days_below_upper, min_band_position_threshold, max_band_position_threshold):
    combined_data = get_combined_data(input_file)
    trades_df = process_trades_day_by_day(combined_data, min_days_below_upper,
                                          min_band_position_threshold, max_band_position_threshold)
...
output_df = process_input_file(input_file, ma_length, upper_multiplier,
                               lower_multiplier, min_days_below_upper,
                               min_band_position_threshold, max_band_position_threshold)
